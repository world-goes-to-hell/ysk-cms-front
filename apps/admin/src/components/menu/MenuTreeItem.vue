...
const emit = defineEmits<{
  ...
  (e: 'open-create-dialog', parent: MenuItem): void
  (e: 'open-edit-dialog', menu: MenuItem): void
  (e: 'delete-menu', menu: MenuItem): void
  (e: 'drag-change'): void
  (e: 'update:children', children: MenuItem[]): void
}>()

const isExpanded = computed(() => props.expandedKeys.has(props.menu.id))

// ref로 children 관리 (vuedraggable이 직접 수정 가능)
const localChildren = ref<MenuItem[]>([...(props.menu.children || [])])
...
const handleChildrenUpdate = (index: number, newChildren: MenuItem[]) => {
